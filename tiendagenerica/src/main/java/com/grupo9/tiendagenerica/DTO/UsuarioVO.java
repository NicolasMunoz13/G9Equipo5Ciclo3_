package com.grupo9.tiendagenerica.DTO;

import java.io.Serializable;

//Todas las clases entidad deben ser serializables y deben ser encapsuladas
//Con la serializacion permite la formacion de un archivo JSON con mas facilidad
public class UsuarioVO implements Serializable {
	
	//idntificador unico de las clases entidad
	private static final long serialVersionUID = 1L;
	
	private Integer cedula_usuario;
	private String email_usuario;
	private String nombre_usuario;
	private String password;
	private String usuario;
	
	//Generacion de getters y setters
	
	/**
	 * Metodo que permite ver el valor de la cedula
	 * @return the cedula_usuario
	 */
	public Integer getCedula_usuario() {
		return cedula_usuario;
	}
	/**
	 * Metodo que permite ver el cambio de la cedula
	 * @param cedula_usuario the cedula_usuario to set
	 */
	public void setCedula_usuario(Integer cedula_usuario) {
		this.cedula_usuario = cedula_usuario;
	}
	/**
	 * Metodo que permite ver el valor de la cedula
	 * @return the email_usuario
	 */
	public String getEmail_usuario() {
		return email_usuario;
	}
	/**
	 * Metodo que permite ver el cambio de la cedula
	 * @param email_usuario the email_usuario to set
	 */
	public void setEmail_usuario(String email_usuario) {
		this.email_usuario = email_usuario;
	}
	/**
	 * Metodo que permite ver el valor de la cedula
	 * @return the nombre_usuario
	 */
	public String getNombre_usuario() {
		return nombre_usuario;
	}
	/**
	 * Metodo que permite ver el cambio de la cedula
	 * @param nombre_usuario the nombre_usuario to set
	 */
	public void setNombre_usuario(String nombre_usuario) {
		this.nombre_usuario = nombre_usuario;
	}
	/**
	 * Metodo que permite ver el valor de la cedula
	 * @return the password
	 */
	public String getPassword() {
		return password;
	}
	/**
	 * Metodo que permite ver el cambio de la cedula
	 * @param password the password to set
	 */
	public void setPassword(String password) {
		this.password = password;
	}
	/**
	 * Metodo que permite ver el valor de la cedula
	 * @return the usuario
	 */
	public String getUsuario() {
		return usuario;
	}
	/**
	 * Metodo que permite ver el cambio de la cedula
	 * @param usuario the usuario to set
	 */
	public void setUsuario(String usuario) {
		this.usuario = usuario;
	}

}
